\chapter{Linear preferential trees}\label{chapter linear preferential trees}
% LTeX: language=de-DE
...
\label{Gewichte Satz}
   Mit den linearen Gewichten $w_k = \chi k + p $, hat ein preferential Baum $T$ mit $m$ Knoten ein Gesamtgewicht $w(T) = (m-1)\chi + m\rho = m(\chi + \rho) - \chi$.
\end{theorem}
\begin{proof}
...
\begin{theorem} \cite[Theorem 3.2]{janson2019random}
    ...
    Sei $(T_n)_1^\infty =(T_n^{\chi,\rho})_1^\infty$ eine Folge von linear preferential attachment trees, mit den Kindern von der Wurzel in der Reihenfolge ihres Erscheinens beschriftet. Sei $N_j(n) := |T^j_n|$, die Größe des j-ten \PsubT von $T_n$. Dann gilt 
    \[ 
...
\begin{section}{Konstruktion von Split Bäumen}
